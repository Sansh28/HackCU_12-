...
- `savant-frontend`: Main research cockpit for uploads, QA, graph exploration, citations, and voice.
- `savant-backend`: API orchestration layer for ingestion, retrieval, graph generation, persistence, and optional payment checks.
- `savant-extension`: Lightweight side panel for supported research websites that turns live paper context into a concept tree.
