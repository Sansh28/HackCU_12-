...
- `savant-backend/`: FastAPI service for ingestion, retrieval, graph generation, session storage, and optional payment verification
- `savant-frontend/`: Next.js web client for paper upload, chat, citations, voice, and graph exploration
- `savant-extension/`: Chrome side-panel extension that extracts context from supported research pages and renders concept graphs
...
- `savant-backend/main.py` is still the dominant architectural bottleneck and should be split into routers/services
