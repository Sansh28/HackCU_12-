...
@router.post("/upload")
async def upload_document(file: UploadFile = File(...), owner_id: str = Depends(get_owner_id)):
...
async def get_document_context(doc_id: str, owner_id: str = Depends(get_owner_id)):
    services.require_database()
    docs = await store.documents_collection.find(
        owner_filter(owner_id, doc_id=doc_id),
...
@router.post("/query")
async def query_savant(request: models.QueryRequest, owner_id: str = Depends(get_owner_id)):
    services.require_database()
    ...
    if request.doc_id:
        existing_doc = await store.documents_collection.find_one(owner_filter(owner_id, doc_id=request.doc_id), {"_id": 1})
        if not existing_doc:
            raise HTTPException(status_code=404, detail="Document not found")
    ...
    filter_query = owner_filter(owner_id)
    if request.doc_id:
    ...
    if request.session_id:
        session = await store.sessions_collection.find_one(owner_filter(owner_id, session_id=request.session_id))
        if session:
            ...
            await store.sessions_collection.update_one(owner_filter(owner_id, session_id=request.session_id), {"$set": {"updated_at": now}})
